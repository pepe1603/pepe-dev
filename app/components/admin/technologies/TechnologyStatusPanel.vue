<script setup lang="ts" name="TechnologyStatusPanel">
import { ref, computed, watch } from 'vue'
import { useUpdateTechnologyStatusUseCase } from '~/composables/admin/technologies/usecases/useUpdateTechnologyStatusUseCase'
 ...
 * (NO dependemos directamente de la prop)
 */
...
const { updateStatus, loading, error, success } =
  useUpdateTechnologyStatusUseCase()
...
<template>
  <div class="border border-accented rounded-lg p-5 bg-muted/30 space-y-4">
    ...
    <div v-else class="text-xs text-muted">
      This technology can no longer change status.
    </div>
